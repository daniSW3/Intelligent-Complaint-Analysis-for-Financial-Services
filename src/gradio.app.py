...
This notebook sets up an interactive web interface using Gradio for the complaint-answering chatbot, integrating the RAG pipeline from Task 3. The interface includes a text input, submit button, answer display, source chunks, and a clear button, with optional streaming for better user experience.
"""

# Install required libraries
!pip install -q gradio sentence-transformers faiss-cpu transformers torch
...
from pathlib import Path
import gradio as gr
# from transformers import pipeline # Removed pipeline import
import torch

# Set up paths
VECTOR_STORE_PATH = Path('/content/vector_store')
INDEX_FILE = VECTOR_STORE_PATH / 'faiss_index.bin'
METADATA_FILE = VECTOR_STORE_PATH / 'metadata.pkl'

# Load vector store and metadata (assuming uploaded from Task 2)
index = faiss.read_index(str(INDEX_FILE))
with open(METADATA_FILE, 'rb') as f:
    metadata = pickle.load(f)
...
# Load LLM (using Hugging Face pipeline with GPU support in Colab) - REMOVED
# generator = pipeline('text-generation', model='mistralai/Mixtral-8x7B-Instruct-v0.1', device=0 if torch.cuda.is_available() else -1)

"""## Define RAG Functions

Reuse the retriever and generator functions from Task 3, adapted for the interface.
"""
...
# Generator function with streaming
def generate_answer_stream(question):
    context, retrieved_chunks = retrieve_chunks(question)
    prompt = PROMPT_TEMPLATE.format(context=context, question=question)

    # Generate response using the generate_text function
    answer = generate_text(prompt)

    # Yield the answer as a single chunk for now (streaming implementation can be refined)
    yield answer, retrieved_chunks
...
def chat_interface(question, history):
    ...
    # Generate answer and get chunks
    for answer, retrieved_chunks in generate_answer_stream(question):
        # Append the question and answer to history
        history.append([question, answer])

        # Format source chunks for display
        source_chunks_display = [c['original_text'][:100] + "..." for c in retrieved_chunks]

        # Yield the updated history and source chunks
        yield "", history, source_chunks_display
...
Create the web interface with input, submit button, answer display, sources, and clear functionality.
"""

# Interface function (adapted for Blocks)
def chat_interface(question, history):
    history = history or []
    if not question:
        return history, [], ""
    ...
    # Generate answer and get chunks
    answer, retrieved_chunks = next(generate_answer_stream(question))
    ...
    # Return the updated history, answer, and source chunks
    return history, source_chunks_display, "" # Return empty string for question textbox


# Clear function (adapted for Blocks)
def clear_chat():
    return [], [], "" # Clear history, source chunks, and question textbox

# Create Gradio interface using Blocks
with gr.Blocks() as demo:
    gr.Markdown("# CrediTrust Complaint Assistant")
    gr.Markdown("Ask questions about customer complaints. Sources are shown for transparency.")

    chatbot = gr.Chatbot(label="Conversation History")
    source_chunks_display = gr.Textbox(label="Source Chunks (first 100 chars each)", interactive=False)

    with gr.Row():
        question_textbox = gr.Textbox(lines=2, placeholder="Type your question here...", label="Your Question")
    ...
    submit_btn.click(
        fn=chat_interface,
        inputs=[question_textbox, history_state],
        outputs=[chatbot, source_chunks_display, question_textbox],
        # This is not used for streaming in this Blocks setup, but would be for streaming outputs
        # live=False
    )
    ...
    clear_btn.click(
        fn=clear_chat,
        inputs=[],
        outputs=[chatbot, source_chunks_display, question_textbox, history_state]
    )
...
"""## Report: Interactive Chat Interface

### Screenshots/GIF
- **Capture Screenshots**: After launching the interface, take screenshots showing:
  - The initial interface with the input box and examples.
  - A submitted question with the streaming answer and source chunks.
  - The interface after using the "Clear" button.
- **Create GIF**: Use a screen recording tool (e.g., Loom, GIF Brewery) to record a demo (e.g., asking a question, seeing the answer stream, and clearing). Save as a `.gif` file.
- **Include in Report**: Embed these in your final report (e.g., using Markdown `![Screenshot](path/to/screenshot.png)`).

### Notes
- The interface uses Gradio's streaming capability to display answers token-by-token, enhancing user experience.
- Source chunks are shown for transparency, building trust by allowing users to verify the response.
- The "Clear" button resets the conversation, improving usability.
- Ensure the vector store files (`faiss_index.bin`, `metadata.pkl`) from Task 2 are uploaded to Colab's `/content/vector_store/` before running.
"""